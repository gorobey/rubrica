...
class estudiantes extends MySQL
{
	...
	function truncateFloat($number, $digitos) {
		...
		return $this->truncateFloat($resultado, $digitos);
	}
	...
	{
		$consulta = parent::consulta("SELECT * FROM sw_estudiante e, sw_estudiante_periodo_lectivo ep WHERE ep.id_estudiante = e.id_estudiante AND ep.id_periodo_lectivo = $id_periodo_lectivo AND e.es_apellidos = '$apellidos' AND e.es_nombres = '$nombres'");
		return (parent::num_rows($consulta) > 0);
	}
	...
	function es_promocionado($id_estudiante, $id_periodo_lectivo)
	{
		$qry = parent::consulta("SELECT es_promocionado(" . $id_estudiante . ", " . $id_periodo_lectivo . ") AS aprobado");
	...
	function buscarEstudiantesAntiguos($patron)
	{
		...
		if ($num_registros > 0) {
			...
			while($estudiante = parent::fetch_assoc($consulta))
			{
				...
				//*******************************************************************
				$aprobado = $this->es_promocionado($id_estudiante,$this->id_periodo_lectivo - 1);
	...
	{
		$consulta = parent::consulta("SELECT e.id_estudiante, es_apellidos, es_nombres, es_cedula, es_genero, id_paralelo FROM sw_estudiante e, sw_estudiante_periodo_lectivo p WHERE e.id_estudiante = p.id_estudiante AND id_periodo_lectivo = " . $this->id_periodo_lectivo . " AND e.id_estudiante = " . $this->code);
	...
	{
		$consulta = parent::consulta("SELECT cu_nombre, pa_nombre FROM sw_estudiante e, sw_estudiante_periodo_lectivo ep, sw_curso c, sw_paralelo p WHERE ep.id_estudiante = e.id_estudiante AND ep.id_paralelo = p.id_paralelo AND p.id_curso = c.id_curso AND e.id_estudiante = $id_estudiante AND id_periodo_lectivo = $id_periodo_lectivo");
		$estudiante = parent::fetch_assoc($consulta);
		return $estudiante["cu_nombre"] . " " . $estudiante["pa_nombre"];
	}
	...
	function listarEstudiantes()
	{
		...
		if($num_total_registros>0)
		{
			...
			while($estudiantes = parent::fetch_assoc($consulta))
			{
				...
				$cadena .= "<td width=\"24%\" align=\"left\">$cedula</td>\n";
	...
	{
		$qry = "INSERT INTO sw_estudiante (es_apellidos, es_nombres, es_cedula, es_genero) VALUES (";
		...
		if (!$consulta)
			...
			$qry .= $estudiante->max_id_estudiante . ",";
