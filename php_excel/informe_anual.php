...
if($num_total_estudiantes > 0)
{
	...
	while($estudiante = $db->fetch_assoc($estudiantes))
	{
		...
		$query = $db->consulta("SELECT calcular_promedio_anual($id_periodo_lectivo, $id_estudiante, $id_paralelo, $id_asignatura) AS calificacion");
		...
		if($calificacion_anual < 7)
		{
			...
			$objPHPExcel->getActiveSheet()->setCellValue('D'.$fila_base, $calificacion_anual);
			$fila_base++;
		}
	}
}
...
$objWriter->save($baseFilename . " " . $nombreParalelo . " " . $nombreAsignatura . " " . $nombrePeriodoLectivo . ".xls");
...
// Codigo para abrir la caja de dialogo Abrir o Guardar Archivo
	...
	header ("Content-Disposition: attachment; filename=\"" . $baseFilename . " " . $nombreParalelo . " " . $nombreAsignatura . " " . $nombrePeriodoLectivo . ".xls" . "\"" );
	readfile($baseFilename . " " . $nombreParalelo . " " . $nombreAsignatura . " " . $nombrePeriodoLectivo . ".xls");
