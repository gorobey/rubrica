...
CREATE DEFINER=`root`@`localhost` FUNCTION `aprueba_todos_remediales`(
	...
	Lazo: LOOP
		...
		IF promedio > 5 AND promedio < 7 THEN -- tiene que rendir el examen supletorio
			...
			IF examen_supletorio < 7 THEN
				...
				SET examen_remedial = (SELECT calcular_examen_supletorio(IdEstudiante,IdParalelo,IdAsignatura,3));
