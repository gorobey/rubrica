...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#cboPeriodosEvaluacion").change(function(e){
			...
			$('#cboClub').val(0);
			$("#mensaje").html("Debe seleccionar un Club...");
			$("#cboClub").focus();
		});
		$("#cboClub").change(function(e){
			e.preventDefault();
			var id_club = $("#cboClub").val();
			if(id_club==0) {
				$("#mensaje").html("Debe seleccionar un Club...");
			} else {
	...
	function cargarAportesEvaluacion(iDesplegar)
	{
		...
		if(id_periodo_evaluacion==0) {
			...
			$.ajax({
				type: "POST",
				url: "aportes_evaluacion/listar_aportes_estados_clubes.php",
				data: "id_periodo_evaluacion="+id_periodo_evaluacion+"&id_club="+id_club,
	...
	function cerrarAporteEvaluacion(id, estado)
	{
		...
		if(id_periodo_evaluacion==0) {
			...
			if (cerrar) {
				...
				$.ajax({
					type: "POST",
					url: "aportes_evaluacion/cerrar_aporte_evaluacion_club.php",
					data: "id_aporte_evaluacion="+id+"&id_club="+id_club+"&estado="+estado,
					success: function(resultado){
	...
	function actualizarAporteEvaluacion(id, nombre, fecha_apertura, fecha_cierre)
	{
		...
		$.ajax({
			type: "POST",
			url: "aportes_evaluacion/actualizar_fechas_aporte_evaluacion_clubes.php",
			data: "id_aporte_evaluacion="+id+"&id_club="+id_club+"&ap_nombre="+nombre+"&fecha_apertura="+txt_fecha_apertura+"&fecha_cierre="+txt_fecha_cierre,
			success: function(resultado){
				$("#mensaje").css("color","blue");
				$("#mensaje").html(resultado);
				cargarAportesEvaluacion(true);
	...
	<div id="barra_principal">
      <table id="tabla_navegacion" border="0" cellpadding="0" cellspacing="0">
         <tr>
            ...
            <td width="5%" class="fuente9" align="center"> &nbsp;Club:&nbsp; </td>
            <td width="5%"> <select id="cboClub" class="fuente8"> <option value="0"> Seleccione... </option> </select> </td>
