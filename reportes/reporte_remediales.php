<?php
	...
	$consulta = $db->consulta("SELECT e.id_estudiante, es_apellidos, es_nombres FROM sw_estudiante e, sw_estudiante_periodo_lectivo p WHERE e.id_estudiante = p.id_estudiante AND p.id_paralelo = $id_paralelo AND es_retirado = 'N' ORDER BY es_apellidos, es_nombres ASC");
				...
				if($total_asignaturas>0)
				{
					...
					while ($asignatura = $db->fetch_assoc($id_asignaturas))
					{
						...
						if($promedio_periodos >= 7 || 7 - $promedio_periodos < 0.01) {
							...
							if($calificacion < 7) {
								// Aquí obtengo la calificación del examen remedial
