<?php
	...
	{
		$contador = 0;
		while($p = $db->fetch_assoc($consulta))
		{
			...
			$nombre_completo = utf8_decode($p["es_apellidos"])." ".utf8_decode($p["es_nombres"]);
...
//					$promedio_quimestral = $calificacion["promedio"];
					...
					$promedio_quimestral = $periodo_evaluacion->calcularPromedioQuimestre($id_periodo_evaluacion, $id_estudiante, $id_paralelo, $id_asignatura);
					...
					{
						if ($promedio_quimestral < 7 && $promedio_quimestral > 0)
						{
							$pdf->Cell(13,6,number_format($promedio_quimestral,2),1,0,'C');
					    }
					    else
					    {
							$promedio_quimestral=($promedio_quimestral==0)?'S/N':' ';
							$pdf->Cell(13,6,$promedio_quimestral,1,0,'C');
						}
					}
