...
class paralelos extends MySQL
{
	...
	{
		$consulta = parent::consulta("SELECT e.id_estudiante, c.id_curso, pa.id_paralelo, pa.id_asignatura, e.es_apellidos, e.es_nombres, es_retirado, as_nombre, cu_nombre, pa_nombre FROM sw_paralelo_asignatura pa, sw_estudiante_periodo_lectivo ep, sw_estudiante e, sw_asignatura a, sw_curso c, sw_paralelo p WHERE pa.id_paralelo = ep.id_paralelo AND pa.id_periodo_lectivo = ep.id_periodo_lectivo AND ep.id_estudiante = e.id_estudiante AND pa.id_asignatura = a.id_asignatura AND pa.id_paralelo = p.id_paralelo AND p.id_curso = c.id_curso AND pa.id_paralelo = " . $this->id_paralelo . " AND pa.id_asignatura = " . $this->id_asignatura . " AND es_retirado <> 'S' ORDER BY es_apellidos, es_nombres ASC"); //LIMIT $inicio, $cantidad_registros
		$num_total_registros = parent::num_rows($consulta);
		$cadena = "<table id=\"tabla_calificaciones\" class=\"fuente8\" width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">\n";
		if($num_total_registros>0)
		{
